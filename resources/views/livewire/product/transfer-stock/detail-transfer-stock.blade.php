    ...
    <div class="p-6 border">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                @if ($transferStock?->transferProducts)
                    @foreach ($transferStock?->transferProducts as $transferProduct)
                        <tr class="{{ $loop->index % 2 === 0 ? 'bg-gray-50 dark:bg-gray-900' : '' }}">
                            ...
                            <td class="px-4 py-2 text-sm text-gray-500 whitespace-nowrap dark:text-gray-200">
                                {{ $transferProduct->productStock->product->name }}
                            </td>
                            <td class="px-4 py-2 text-sm text-gray-500 whitespace-nowrap dark:text-gray-200">
                                {{ $transferProduct->productStock->color->name }}
                            </td>
                            <td class="px-4 py-2 text-sm text-gray-500 whitespace-nowrap dark:text-gray-200">
                                {{ $transferProduct->productStock->size->name }}
                            </td>
                            <td class="px-4 py-2 text-sm text-gray-500 whitespace-nowrap dark:text-gray-200">
