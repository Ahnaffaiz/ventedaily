<div>
    <x-modal wire:model="isOpen" title="{{ $expense ? 'Edit Expense' : 'Create Expense' }}"
        saveButton="{{ $expense ? 'update' : 'save' }}" closeButton="closeModal">
        <form>
            <x-input-text type="date" id="date" name="date" title="Expense Date" />
                    ...
                    <thead>
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-4 text-sm font-medium text-gray-500 text-start"
                                wire:click="sortByColumn('cost_id')">
                                Cost Name
                                @if ($sortBy === 'cost_id')
