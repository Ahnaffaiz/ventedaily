@props(['title' => 'Modal Title', 'isOpen' => false, 'closeButton' => null, 'saveButton' => null])
...
<div x-data="{ open: @entangle($attributes->wire('model')) }" x-show="open"
    ...
    <div
        class="flex flex-col m-3 transition-all duration-300 ease-in-out bg-white rounded shadow-sm sm:max-w-lg sm:w-full sm:mx-auto dark:bg-gray-800">
        ...
        <div class="flex items-center justify-end gap-2 p-4 border-t dark:border-slate-700">
            ...
            <button class="text-white btn bg-primary" wire:click="{{ $saveButton }}">Save Changes</button>
        </div>
    </div>
</div>
