<div>
    ...
    <x-modal wire:model="isOpen" title="{{ $category ? 'Edit ' . $category?->name : 'Create Category' }}"
        ...
        <div class="gap-4">
            @if ($categoryPreviews)
                <button wire:click="resetCategoryPreview" class="inline gap-2 transition-all btn bg-danger/25 text-danger hover:bg-danger hover:text-white" wire:target="resetCategoryPreview" wire:loading.attr="disabled">
                    <div class="flex gap-2" wire:loading.remove wire:target="resetCategoryPreview">
                        <i class="ri-refresh-line"></i>
                        Reset Form
                    </div>
                    <div class="flex gap-2" wire:loading wire:target="resetCategoryPreview">
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @foreach ($categoryPreviews as $category)
            ...
            @else
                <x-input-text type="file" name="category_file" id="category_file" title="Upload File Excel" accept=".xlsx, .xls"/>
            @endif
        </div>
        @else
            <form>
                            ...
                            <tr class="{{ $loop->index % 2 === 0 ? 'bg-gray-100 dark:bg-gray-900' : '' }}">
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex items-center justify-center space-x-3">
                                        <button wire:click="edit({{ $category->id }})"><i
                                                class="ri-edit-circle-line text-primary"></i></button>
                                        <button wire:click="deleteAlert({{ $category->id }})"><i
