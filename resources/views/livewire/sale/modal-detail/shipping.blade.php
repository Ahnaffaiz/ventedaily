                ...
                <td class="px-2 py-2 font-semibold text-md text-gray-950 dark:text-gray-200">
                    @if ($sale?->saleShipping?->status)
                        @if ($sale?->saleShipping?->status == strtolower(App\Enums\ShippingStatus::SIAPKIRIM))
                            <span class="inline-flex items-center gap-1.5 py-0.5 px-1.5 rounded-md text-xs font-medium dark:bg-info text-whitebg-info/10 text-info">
                                {{ ucfirst($sale?->saleShipping?->status) }}
                            </span>
                        @elseif ($sale?->saleShipping?->status == strtolower(App\Enums\ShippingStatus::EKSPEDISI))
                            <span class="inline-flex items-center gap-1.5 py-0.5 px-1.5 rounded-md text-xs font-medium dark:bg-warning text-whitebg-warning/10 text-warning">
                                {{ ucfirst($sale?->saleShipping?->status) }}
                            </span>
                        @elseif ($sale?->saleShipping?->status == strtolower(App\Enums\ShippingStatus::SELESAI))
                            <span class="inline-flex items-center gap-1.5 py-0.5 px-1.5 rounded-md text-xs font-medium dark:bg-success text-whitebg-success/10 text-success">
                                {{ ucfirst($sale?->saleShipping?->status) }}
                            </span>
                        @endif
                    @else
                        <span class="inline-flex items-center gap-1.5 py-0.5 px-1.5 rounded-md text-xs font-medium dark:bg-secondary text-whitebg-secondary/10 text-secondary">
                            Cashier
                        </span>
                    @endif
                </td>
