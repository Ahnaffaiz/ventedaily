<div>
    @php
        ...
        if($isImport) {
            ...
            $saveButton = $color ? 'update' : 'save';
        }
    @endphp
    <x-modal wire:model="isOpen" title="{{ $color ? 'Edit ' . $color?->name : 'Create Color' }}"
        saveButton="{{ $saveButton }}" saveLabel="{{ $saveLabel }}" closeButton="closeModal">
        @if ($isImport)
        <div class="gap-4">
            @if ($colorPreviews)
                <button wire:click="resetColorPreview" class="inline gap-2 transition-all btn bg-danger/25 text-danger hover:bg-danger hover:text-white" wire:target="resetColorPreview" wire:loading.attr="disabled">
                    <div class="flex gap-2" wire:loading.remove wire:target="resetColorPreview">
                        <i class="ri-refresh-line"></i>
                        Reset Form
                    </div>
                    <div class="flex gap-2" wire:loading wire:target="resetColorPreview">
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @foreach ($colorPreviews as $color)
            ...
            @else
                <x-input-text type="file" name="color_file" id="color_file" title="Upload File Excel" accept=".xlsx, .xls"/>
            @endif
        </div>
        @else
        <form>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tr class="{{ $loop->index % 2 === 0 ? 'bg-gray-100 dark:bg-gray-900' : '' }}">
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex items-center justify-center space-x-3">
                                        ...
                                        <button wire:click="deleteAlert({{ $color->id }})">
                ...
                @else
                    <div class="text-center">
                        ...
                        <p class="my-5 text-base">No Color Found</p>
                    </div>
                @endif
            </table>
        </div>
            ...
            <div class="flex justify-between">
                <div class="flex flex-col">
                    ...
                    <div class="mt-2 text-sm text-gray-600">
                        Showing {{ $colors->firstItem() ?? 0 }} to {{ $colors->lastItem() ?? 0 }} of {{ $colors->total() }} entries
                    </div>
                </div>
