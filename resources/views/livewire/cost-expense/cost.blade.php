<div>
    <x-modal wire:model="isOpen" title="{{ $cost ? 'Edit ' . $cost?->name : 'Create Cost' }}"
        saveButton="{{ $cost ? 'update' : 'save' }}" closeButton="closeModal">
        <form>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        @foreach ($costs as $cost)
                            <tr class="{{ $loop->index % 2 === 0 ? 'bg-gray-100 dark:bg-gray-900' : '' }}">
                                <th class="px-4 py-4 text-sm text-gray-500 whitespace-nowrap dark:text-gray-200">
                                    {{($costs->currentpage() - 1) * $costs->perpage() + $loop->index + 1}}
                                </th>
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex items-center justify-center space-x-3">
                                        <button wire:click="edit({{ $cost->id }})"><i
                                                class="ri-edit-circle-line text-primary"></i></button>
                                        <button wire:click="deleteAlert({{ $cost->id }})"><i
                ...
                @else
                    <div class="text-center">
                        ...
                        <p class="my-5 text-base">No Cost Found</p>
                    </div>
                @endif
            </table>
        </div>
            ...
            <div class="flex flex-col items-center md:flex-row md:justify-between gap-4">
                <div class="flex flex-col items-center md:items-start">
                    <div class="mt-2 text-sm text-center md:text-left text-gray-600">
                        Showing {{ $costs->firstItem() ?? 0 }} to {{ $costs->lastItem() ?? 0 }} of {{ $costs->total() }} entries
                    </div>
                </div>
