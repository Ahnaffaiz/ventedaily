...
<body style="font-family: courier; font-size: 12px;width: 100%;">
    ...
    <H3>NOTA PEMBAYARAN<br>No. PAY-{{date('Y')}}-{{date('m')}}-{{$payment->id}}</H3>
    <table style="margin-top: -5px;width: 80%">
        <tr>
            <td>Pelanggang</td>
            <td>:</td>
            <td colspan="2">[ {{ $payment->sale->customer->id}} ] {{$payment->sale->customer->name}}</td>
        </tr>

        <tr>
            <td>Term</td>
            <td>:</td>
            <td colspan="2">{{ $payment->sale->termOfPayment->name }} </td>
        </tr>
        <tr style="font-size: 12px;">
            <td colspan="3" style="text-align: left;">Nilai Invoice (Rp.)</td>
            <td style="text-align: right;">{{ number_format($payment->sale->total_price) }}</td>
        </tr>
        <tr>
            <td colspan="4">..................................</td>
        </tr>
        ...
        <tr style="font-size: 12px;">
            ...
            <td style="text-align: right;"><strong>{{ number_format($payment->sale->outstanding_balance) }}</strong>
            </td>
        </tr>
        <tr>
            <td colspan="4">..................................</td>
        ...
        <tr>
            <td colspan="4" style="text-align: left;">{{ $payment->sale->customer->name }}</td>
        </tr>
        <tr>
